...
### Added
- **One-Shot Deploy Script** — `scripts/deploy.sh` puts the checkout into service on the host it runs on: preflight → `deno task verify` → create the no-shell `ptweb` service account → ownership (owner writes, service reads via group) → install the env file + provision `ADMIN_SESSION_SECRET` → populate the module cache for `--cached-only` → render + install the systemd unit (host paths patched in with `sed`, no per-host commits) → health poll on `/api/health` → ACME bootstrap + `certbot` + renewal reload hook → install the nginx vhost, probe snippet, and catch-all default-drop → install the fail2ban filter + jail and verify the jail is reading the access logs. Idempotent, stops at the first failure, nothing is installed until verification passes. Flags: `--skip-verify`, `--skip-nginx`, `--skip-certbot`, `--skip-fail2ban`, `--staging`, `--force-renewal`. Modelled on `grenas405/pmd-web-app`'s `scripts/deploy.sh`.
- **fail2ban + nginx hardening files** — `fail2ban/filter.d/nginx-probes.conf`, `fail2ban/jail.local` (bans PHP/WordPress/dotfile scanners off the access log; `backend = polling`), `nginx/snippets/deny-probes.conf` (444 on probe paths, kept in the log for fail2ban), and `nginx/00-default-drop` (closes the connection on any unmatched `Host`, and owns the `ssl`/`http2` options for the `:443` socket).
- **`systemd/praxedis-technologies.env.example`** — per-host overrides template, installed to `/etc/praxedis-technologies/praxedis-technologies.env`.
- **`deno task verify`** — `deno fmt --check && deno task check && deno task test` in one task, used by the deploy script and handy locally.

### Changed
- **systemd unit hardened, dedicated service account** — `systemd/praxedis-technologies.service` now runs as a no-login system user (`ptweb`) that only reads the checkout through its group, not as the checkout owner. `Type=exec`, full sandbox (`ProtectSystem=strict`, `ProtectHome=tmpfs` + `BindReadOnlyPaths` for the code, `SystemCallFilter=@system-service`, empty `CapabilityBoundingSet`, `ProtectProc=invisible`, …), `--cached-only`, KV in `StateDirectory=/var/lib/praxedis-technologies`, module cache in `CacheDirectory`. `EnvironmentFile` moved to `/etc/praxedis-technologies/praxedis-technologies.env`. Installed as a **system** unit (`/etc/systemd/system`), not a user unit.
- **`nginx/praxedistechnologies.com.conf`** — `listen 443` is now bare (the `ssl`/`http2` socket options come from `00-default-drop`); the vhost `include`s `snippets/deny-probes.conf`.
- **`scripts/setup_vps_admin.sh`** — seeds the KV as root then hands the file to `ptweb:ptweb` so the running unit can write it; env-file default follows the unit to `/etc/praxedis-technologies/praxedis-technologies.env`; adds `SERVICE_USER`/`SERVICE_GROUP` overrides.
...
### Removed
- **Legacy Inbound URL Redirects** — removed redirects for `/about.html`, `/case-study.html`, `/heavenly-roofing`, and `/heavenly-roofing.html`; these now return the themed 404 page
